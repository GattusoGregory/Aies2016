...
if($rows == 0)
{
	...
	{
		$id = 1;
		header("refresh:5;url=../starterzone5.php?link=$id");	
	}
		else{		
				$sql = "SELECT * FROM slidezone5 WHERE row = $id;";
				$query = mysqli_query($bdd,$sql);
				$rows = mysqli_num_rows($query);
				if($rows == 0)
				{
					$id = $id + 1;
					header("Location:../starterzone5.php?link=$id");
				}
					else{
